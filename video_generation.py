""" This script is to generate the video of a scenario with json format. 
Parameter to set: PATH (path of input files (folder)), output_path (output direction) and DPI (quality of video generated). 
Notice: Files used should be preprocessed by simulator Nocturne or generated through ctrl-sim. 
If you want to 
"""
...
PATH = r"C:\Users\cwang76\Downloads\viz_res\viz_results_050920255"    # input path (folder)
output_path = r"C:\Users\cwang76\Downloads\ctrl-sim\video22092025"      # output path
DPI = 200   # a bigger value could slow the speed of generation

# For needs of generate video with specific agents 
# True: chose the agents presented in scene manually in "agent_defined";  False: all agents will present in the scene
DEFINE_AGENT = False    
agent_defined = [17, 4, 5, 0, 1, 2, 6, 9, 46]     # agent id that shows in the video 
# NOTE: Use agent_id.py to generate image with ids of agents 
focal_id = []     # agent_id that focals (leave it empty if there is no agent to focus)
...
def generate_video_frames(data, name, output_dir, dpi):
    ...
    for n in range(len(agents_data)):
        if n in agent_id:
            ...
            for j in range(len(ag_existence)):
                if x_values[j] == -10000 or y_values[j] == -10000:
                    ag_existence[j] = 0
    ...
    #     print(f"the last timestep agent is {last_timestep_agent}")
    ...
    for e, t in tqdm(enumerate(timesteps)):
        ...
        for a in range(len(coordinates)):
            ...
            # Draw the goal (only for agent_id defined)
            for k in range(len(goals)):
                color = agents_color[k]
                lw = 1/((x_max - x_min)/70)
                if k in focal_id:
                    plt.scatter(goals[k, 0, 0], goals[k,0,1], color = color, s=int(35/((x_max - x_min)/70)), zorder=10, edgecolor='black', linewidth=lw)
...
def generate_video(data, name, output_dir, dpi=DPI, delete_images=True):
...
def main():
    ...
    for e, file in enumerate(files):
        ...
        generate_video(data, imagename, output_path)    

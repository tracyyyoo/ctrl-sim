...
def generate_video_frames(data, name, output_dir, dpi):
    ...
    agent_ctrlsim = []   # find the interesting agent
    agents_color = []   # color list for each agent
   ...
   # agent_color = '#ffde8b'
    ...
    for e, t in tqdm(enumerate(timesteps)):
        ...
        # print only the goal of agent controlled by ctrl-sim
        for a in range(len(goals)):
            color = agents_color[a]
            lw = 1/((x_max - x_min)/70)
            if agents_data[a]['is_ctrl_sim']:
                plt.scatter(goals[a, 0, 0], goals[a,0,1], color = color, s=int(35/((x_max - x_min)/70)), zorder=10, edgecolor='black', linewidth=lw)
...
def generate_video(data, name, output_dir, dpi=150, delete_images=True):

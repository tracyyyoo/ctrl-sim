...
class PolicyEvaluator(Evaluator):
    def __init__(self, cfg, policy):
        ...
        with open(os.path.join(cfg.dataset_root, 'test_filenames_roundabout_carmaker.pkl'), 'rb') as f:
    ...
    def evaluate_policy(self):
        ...
        for enum, file in tqdm(enumerate(files)):
            ...
            if self.cfg.eval.visualize:
                focal_agent_idx = 0
                dir_name = f"viz_{self.test_filenames[file].split('.')[0]}_physics_{self.policy.veh_veh_tilt}_{self.policy.veh_edge_tilt}_{self.policy.goal_tilt}_seed{self.cfg.eval.seed}"
		    ...
		    # add information of agent (controlled by ctrl-sim or not)
                    output_vehicle_data_dict[veh_id]['is_ctrl_sim'] = (veh_id in veh_ids_to_evaluate)
                    if output_vehicle_data_dict[veh_id]['is_ctrl_sim']:
                        focal_agent_idx = veh_id

                road_data = get_road_data(scenario)
                veh_ids = [v.getID() for v in vehicles]
                export_data = {"name": dir_name, "objects": [*output_vehicle_data_dict.values()], "roads": road_data, "focal_agent_idx": focal_agent_idx, "enum": enum, "seed": self.cfg.eval.seed}

                # save the json file
                output_dir = os.path.join("/".join(self.policy.model_path.split("/")[:-1]), "viz_results")
                os.makedirs(output_dir, exist_ok=True)
                output_filename = os.path.join(output_dir, f"viz_result_file_{enum}_{self.cfg.eval.seed}.json")
                json_serialize_data = convert_ndarray_to_list(export_data)
                with open(output_filename, 'w') as f:
                    json.dump(json_serialize_data,f)
                print(f"scene{enum}'s json file saved in {output_dir}")
                video_name = f"{enum}_seed{self.cfg.eval.seed}"
                generate_video(export_data, video_name, self.cfg.eval.movie_path)
